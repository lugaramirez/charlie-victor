...
\begin{itemize}
  \item Migrated login handling out of the monolith; chose an event-based design over direct service coupling, enabling other teams to add post-login behaviour autonomously without coordination overhead.
  \item Demonstrated test-gated automatic releases on a legacy service; the approach spread organically, leading to an invitation to join the 4-person company-wide quality community.
...
{\bodytext{Recruited as Tech Lead for the integration team; promoted to lead the platform team after identifying that the architecture itself --- not the teams --- was the source of the company's growth problems.}}

\headedsubsection % Job title entry for the current employer
{Tech Lead}
{Sep '21 -- Oct '23}
{\bodytext{
\textit{The situation: a Germany expansion attempt that exposed the microservices architecture collapsing under its own weight --- a feature as simple as showing a doctor their patient list took 6 to 8 months to ship, infrastructure costs exceeded \$2M annually, deployments were quarterly, and onboarding a new developer took nearly a year.}
\begin{itemize}
  \item Diagnosed the shared commons library as the most immediate drag --- accumulated RestTemplate clients for every service in the landscape, loaded by every microservice regardless of need; stripped it down, cutting individual service footprint by 20\%+.
  \item Standardized Git workflows, code review practices, and \acr{CI/CD} pipelines across all backend teams.
  \item Bet on a modular monolith over the prevailing microservices orthodoxy; consolidated 70+ services into a single event-driven architecture with 5 bounded domains and 3 anti-corruption layers, applying Domain-Driven Design throughout.
...
{\textsc{Amsterdam, The Netherlands}} {
{\bodytext{Joined as a Backend Trainer; promoted to R\&D after identifying that the product's integration approach --- building scenario-specific forms for each banking workflow --- would not scale.}}

\headedsubsection
{Tech Lead}
{May '20 -- Aug '21}
{\bodytext{
\textit{The two core problems: an integration strategy built around one-off scenario forms with no reusable components across them, and a release process so fragile that every deployment broke something.}
\begin{itemize}
  \item Diagnosed that the scenario forms shared underlying integration components (identity verification, \acr{SMS}/email, document signing) being reimplemented repeatedly; redesigned the approach around composable connectors --- personally built 5, the template grew to 12 with the team, and enabled 20+ connectors after departure.
...
{\bodytext{Consulting firm serving US clients from Paraguay. Brought in to solve a cross-system communication problem every other engineer had declined, with a one-month deadline.}}

\headedsubsection
{Team Lead}
{Feb '17 -- Dec '19}
{\bodytext{
\textit{Two sequential problems: a Java call center application that couldn't communicate with a \acr{COBOL} mainframe (one month, declared impossible by everyone else), followed by a request to ``update'' the call center that warranted a full architectural diagnosis.}
\begin{itemize}
...
{Santco \acr{SRL}}
{\textsc{Asuncion, Paraguay}} {
{\bodytext{Software company serving high-profile Paraguayan clients in cattle trading, logistics, and manufacturing --- all with no existing software for their most critical operations and zero tolerance for downtime.}}

\headedsubsection
{Software Engineer \& Team Lead}
{Nov '14 -- Mar '16}
{\bodytext{
\textit{The decision was to introduce Scrum before building anything --- recognizing that delivery reliability, not technical capability, was the actual constraint.}
\begin{itemize}
